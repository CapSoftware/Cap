...
export async function importLoomVideoForOwner({
	loomUrl,
	orgId,
	ownerId,
}: {
	...
	const existing = await db()
		.select({
			importedVideoId: importedVideos.id,
			videoId: videos.id,
		})
		.from(importedVideos)
		.leftJoin(
	...
	for (const staleImport of existing) {
		await db()
			...
			.where(
				and(
					...
					eq(importedVideos.source, "loom"),
					eq(importedVideos.sourceId, loomVideoId),
					eq(importedVideos.id, staleImport.importedVideoId),
				),
			);
	}
...
async function getOrCreateImportSpace({
	...
	if (existingSpace) {
		const value = { id: existingSpace.id, name: existingSpace.name };
	...
	const value = { id: spaceId, name: normalizedName };
	spaceCache.set(cacheKey, value);
	return value;
}
...
export async function importLoomCsvForUser({
	rows,
	orgId,
	user,
}: {
