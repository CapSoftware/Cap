...
describe("recording verification object reads", () => {
	...
	it("allows only an audio derivative bound to the current original", async () => {
		const source = "owner/video/.recording/outputs/generation/original.mp4";
		const key = "owner/video/.recording/outputs/audio-quality-v3/published.mp4";
		mocks.token = null;
		mocks.video.source = {
			type: "desktopMP4",
			outputKey: source,
