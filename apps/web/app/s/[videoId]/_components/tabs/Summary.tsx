	...
	const handleRetry = async () => {
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(data.error || "重试 AI 生成失败");
			}

			window.location.reload();
		} catch (error) {
			setRetryError(
				error instanceof Error ? error.message : "重试 AI 生成失败",
			);
		} finally {
			setIsRetrying(false);
		}
				...
				<div className="space-y-4 max-w-sm">
					<div className="p-6 bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg border border-blue-100">
						...
						<p className="mb-4 text-sm leading-relaxed text-gray-600">
							升级到 Cap Pro，即可使用自动标题、视频摘要和智能章节生成等 AI
							功能。
						</p>
						<Button
							href="/pricing"
		...
		<div className="flex flex-col h-full">
			<div className="overflow-y-auto flex-1">
				<div className="p-4 space-y-6">
					{aiData?.summary && (
						<div>
							<h3 className="text-lg font-medium">摘要</h3>
							<div className="mb-2">
					...
					{aiData?.chapters && aiData.chapters.length > 0 && (
						<div className={aiData?.summary ? "mt-6" : ""}>
							<h3 className="mb-2 text-lg font-medium">章节</h3>
