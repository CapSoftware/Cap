...
export const SettingsDropdown = () => {
  ...
  const notificationOptions: NotificationOption[] = [
    { icon: faVideo, label: "Recordings" },
    { icon: faCommentSlash, label: "Comments" },
    { icon: faEyeSlash, label: "Views" },
    { icon: faThumbsUp, label: "Reactions" }
  ];
  return (
            ...
            {showPauseSubmenu && (
              ...
              >
                {notificationOptions.map((option, index) => (
                  <div 
                    key={index}
                    className="flex w-full items-center gap-2 px-2 py-1 text-[13px] text-gray-11 rounded-lg group hover:bg-gray-3 cursor-pointer outline-none"
                  >
