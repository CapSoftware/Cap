...
> = ({ open, onClose, organizationId, organizationName, onVideosAdded }) => {
  ...
  const { data: organizationVideoIds } = useQuery({
    queryKey: ["organization-video-ids", organizationId],
    queryFn: async () => {
      const result = await getOrganizationVideoIds(organizationId);
      if (!result.success) {
        throw new Error(result.error);
      }
      return result.data;
  ...
  const addVideosMutation = useMutation({
    mutationFn: (videoIds: string[]) =>
      addVideosToOrganization(organizationId, videoIds),
    onSuccess: (result) => {
      if (result?.success) {
        ...
        toast.error(result?.error || "Failed to add videos to organization");
      }
    },
    onError: (error) => {
      toast.error("Failed to add videos to organization");
          ...
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900 truncate">
                Add videos to {organizationName}
              </h2>
              ...
              <div className="flex flex-col items-center justify-center h-64 text-center">
                ...
                <p className="text-gray-600 max-w-sm">
                  {searchTerm
                    ...
                    : "Record some videos first to add them to this organization"}
                </p>
              </div>
            ) : (
                ...
                {filteredVideos.map((video) => (
                  <VideoCard
                    ...
                    isAlreadyInOrganization={
                      organizationVideoIds?.includes(video.id) || false
                    }
                  />
                ))}
              </div>
...
const VideoCard: React.FC<VideoCardProps> = ({
  ...
  return (
    <div
      onClick={isAlreadyInOrganization ? undefined : onToggle}
      className={clsx(
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <h3
            className={clsx(
              ...
              isAlreadyInOrganization ? "text-gray-600" : "text-gray-900"
            )}
          >
            {video.name}
          </h3>
        ...
        <div
          className={clsx(
            ...
            isAlreadyInOrganization ? "text-gray-500" : "text-gray-500"
          )}
        >
