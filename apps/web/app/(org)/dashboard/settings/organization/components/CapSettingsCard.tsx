    ...
    <Card className="flex relative flex-col flex-1 gap-6 w-full min-h-fit">
      ...
      <div className="relative">
        <div className="absolute top-0 left-0 z-[20] rounded-xl flex items-center justify-center w-full h-full backdrop-blur-md bg-zinc-900/20">
          <p className="px-3 py-2 text-sm font-semibold rounded-full border text-gray-12 border-gray-3 bg-gray-3">Coming Soon</p>
        </div>
        <div className="flex gap-4 pb-4 mt-3 border-b border-gray-3">
          {["Notifications", "Videos"].map((setting) => (
            <motion.div key={setting} style={{
              borderRadius: 12
            }} onClick={() => setActiveTab(setting)} className={clsx("relative cursor-pointer")}>
            ...
            <AnimatePresence initial={false}>
              {VideoTabSettings.map((setting, index) => (
                <motion.div
                  ...
                  key={index.toString() + setting.label} className="flex justify-between items-center p-3 rounded-xl border border-gray-4">
