...
class Api extends HttpApi.make("WebEditorProjectBundleDownloadTicketApi").add(
	HttpApiGroup.make("root").add(
		HttpApiEndpoint.post(
			"ticket",
			"/api/editor/sessions/:id/project-bundle/download-ticket",
		)
			...
			handlers.handle("ticket", ({ path, payload }) =>
				Effect.gen(function* () {
					...
					const response = yield* requestMediaEditor(
						`${sessionPath}/project-bundle/download-ticket`,
						{
							method: "POST",
					...
					if (
						...
						url.password ||
						url.hash ||
						url.pathname !== `${sessionPath}/project-bundle/download` ||
