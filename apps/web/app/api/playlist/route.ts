	...
	try {
		...
		if (prefix === null) {
			...
			const generatedPlaylist = await generateMasterPlaylist(
				...
				`${serverEnv.WEB_URL}/api/playlist?userId=${userId}&videoId=${videoId}&videoType=video`,
				audioMetadata
					? `${serverEnv.WEB_URL}/api/playlist?userId=${userId}&videoId=${videoId}&videoType=audio`
					: null,
				video.xStreamInfo ?? ""
			);
