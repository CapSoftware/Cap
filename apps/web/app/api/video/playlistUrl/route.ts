...
export async function GET(request: NextRequest) {
	...
	return new Response(
		JSON.stringify({
			playlistOne: `${serverEnv.WEB_URL}/api/playlist?userId=${video.ownerId}&videoId=${video.id}&videoType=video`,
			playlistTwo: `${serverEnv.WEB_URL}/api/playlist?userId=${video.ownerId}&videoId=${video.id}&videoType=audio`,
		}),
		{
			status: 200,
			headers: {
