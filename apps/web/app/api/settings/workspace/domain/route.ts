...
const getRequiredConfig = async (domain: string) => {
  console.log(
    `[Required Config] Fetching required config for domain: ${domain}`
  );
...
export async function POST(request: NextRequest) {
  ...
  console.log(
    `[POST] Request details - Domain: ${domain}, SpaceId: ${spaceId}`
  );
  ...
  try {
    ...
    if (addDomainResponse.error) {
      ...
      return Response.json(
        { error: addDomainResponse.error.message },
        { status: 400 }
      );
    }
    ...
    if (status.verified) {
      console.log(
        `[POST] Domain verified, updating verification timestamp: ${domain}`
      );
      await db
        .update(spaces)
        .set({
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    if (space.customDomain) {
      console.log(
        `[DELETE] Removing domain from Vercel: ${space.customDomain}`
      );
      await fetch(
        `https://api.vercel.com/v9/projects/${
          process.env.VERCEL_PROJECT_ID
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (status.verified && !space.domainVerified) {
      console.log(
        `[GET] Updating domain verification timestamp: ${space.customDomain}`
      );
      await db
        .update(spaces)
        .set({
      ...
      console.log(
        `[GET] Removing domain verification timestamp: ${space.customDomain}`
      );
      await db
        .update(spaces)
        .set({
