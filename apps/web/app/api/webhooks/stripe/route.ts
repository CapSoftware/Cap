  ...
  if (relevantEvents.has(event.type)) {
    try {
      if (event.type === "checkout.session.completed") {
        ...
        let referenceId;
        let isSpaceCustomer = false;
        if (checkoutSession.metadata?.space_id) {
          isSpaceCustomer = true;
          referenceId = checkoutSession.metadata?.space_id;
        } else {
          referenceId = checkoutSession.client_reference_id;
        }

        if (isSpaceCustomer === false) {
          await db
            .update(users)
            .set({
            ...
            .where(eq(users.id, referenceId));
              ...
              sendEmail({
                email: users.email 
                subject: `Thank you for upgrading to Cap Pro!`,
                react: UpgradeEmail({
                  name: user.name,
