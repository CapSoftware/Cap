...
These keys do not authenticate the Cap CLI. For CLI keys (`cap_cli_`), used with `CAP_API_KEY` in headless environments, see [Set Up Your Agent](/docs/agents/setup#headless-environments-ci-containers-and-remote-sandboxes).
...
Credits are tracked in micro-credits (1 dollar = 100,000 micro-credits). Completed SDK uploads are billed at 5,000 micro-credits per minute using the validated duration with a file-size-based minimum. Billing duration is capped at four hours.
...
| `durationInSecs` | number | Yes | Positive video duration in seconds (validated and used for billing) |
...
Credits are debited when the upload completes. Cap bills 5,000 micro-credits per minute using the greater of the submitted duration and the file-size-based minimum, capped at four hours.
