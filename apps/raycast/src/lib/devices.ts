...
export type SystemProfilerSnapshot = {
	microphones: JsonValue;
	cameras: JsonValue;
};

export async function loadDeviceItems(): Promise<DeviceItem[]> {
	const [microphones, cameras] = await Promise.all([
		readSystemProfilerJson("SPAudioDataType"),
		readSystemProfilerJson("SPCameraDataType"),
	]);

	return buildDeviceItemsFromSystemProfiler({ microphones, cameras });
}

export function buildDeviceItemsFromSystemProfiler({
	microphones,
	cameras,
}: SystemProfilerSnapshot): DeviceItem[] {
	return [
		{
			key: "microphone-off",
...
function loadMicrophones(json: JsonValue): DeviceItem[] {
...
function loadCameras(json: JsonValue): DeviceItem[] {
	const seen = new Set<string>();
	const items: DeviceItem[] = [];
