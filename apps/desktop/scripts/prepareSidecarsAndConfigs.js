...
async function createTauriPlatformConfigs() {
  const srcTauri = path.join(__dirname, "../src-tauri/");
  if (isWindows) {
    console.log("Updating Windows Tauri config...");;
    const config = {
      bundle: {
        resources: {
    ...
    await fs.writeFile(`${srcTauri}/tauri.windows.conf.json`, JSON.stringify(config, null, 2));
  } else {
    console.log("Updating macOS Tauri config...");
    const config = {
      bundle: {
        icon: ["icons/macos/icon.icns"],
      },
    };
    await fs.writeFile(`${srcTauri}/tauri.macos.conf.json`, JSON.stringify(config, null, 2));
  }
}
...
async function main() {
  ...
  await createTauriPlatformConfigs();
}
