...
use cap_desktop_runtime::{LogicalPosition, LogicalSize, PhysicalSize, WebviewWindow};
use ffmpeg::{
    format::{self, Pixel},
    frame,
    software::scaling,
...
pub struct CameraPreviewManager {
    store: Result<Arc<cap_desktop_runtime::Store>, String>,
...
impl CameraPreviewManager {
    pub fn new(app: &cap_desktop_runtime::AppHandle) -> Self {
        Self {
            store: app
                .store("cameraPreview")
...
impl InitializedCameraPreview {
    async fn init_wgpu(
        ...
        let _ = (window, default_state, instance);
        Err(anyhow!(
            "Direct native-window camera rendering is unavailable under Electron"
        ))
    }
}
...
async fn resize_window(
    ...
    window
        .run_on_main_thread({
            let window = window.clone();
            move || {
                let result: cap_desktop_runtime::Result<(u32, u32, f64)> = (|| {
                    if should_move {
