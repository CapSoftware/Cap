    ...
    for (label, window) in app.webview_windows() {
        ...
        let Some(native_id) = webview_window_id(&window) else {
            warn!(
                label = %label,
                title = %title,
                "Excluded Electron window has no native window id"
            );
            continue;
        };
        if Window::from_id(&native_id).is_none() {
            if window.is_visible().unwrap_or(false) {
                warn!(
                    ...
                    label = %label,
                    title = %title,
                    "Excluded Electron window id is not visible to CGWindowList"
                );
            } else {
                debug!(
                    window_id = %native_id,
                    label = %label,
                    title = %title,
                    "Skipping hidden excluded Electron window"
                );
            }
            continue;
        }
        if ids.contains(&native_id) {
            debug!(
                ...
                label = %label,
                title = %title,
                "Excluded Electron window id already resolved"
            );
            continue;
        }
        info!(
            ...
            label = %label,
            title = %title,
            "Resolved excluded Electron window"
        );
        ids.push(native_id);
    }
}
...
#[cfg(target_os = "macos")]
fn webview_window_id(window: &cap_desktop_runtime::WebviewWindow) -> Option<WindowId> {
    window
        .native_window_id()
        .ok()
        .flatten()?
