...
#[cfg(target_os = "macos")]
fn macos_focus_permission_window(app: &cap_desktop_runtime::AppHandle) {
...
#[cfg(target_os = "macos")]
fn macos_activate_permission_request(app: &cap_desktop_runtime::AppHandle) {
    if let Err(err) = app.set_dock_visibility(true) {
        tracing::warn!("Failed to show dock icon for permission request: {err}");
    }

    if let Err(err) = app.set_activation_policy(cap_desktop_runtime::ActivationPolicy::Regular) {
...
#[cfg(target_os = "macos")]
fn macos_sync_activation_policy(app: &cap_desktop_runtime::AppHandle, should_show_dock: bool) {
    let policy = if should_show_dock {
        cap_desktop_runtime::ActivationPolicy::Regular
    } else {
        cap_desktop_runtime::ActivationPolicy::Accessory
    };
...
#[cfg(target_os = "macos")]
fn macos_any_window_fullscreen(app: &cap_desktop_runtime::AppHandle) -> bool {
    app.webview_windows()
        .values()
...
pub(crate) fn prepare_macos_panel_window(
    ...
    if prev == 0 && !macos_any_window_fullscreen(app) {
        if let Err(err) =
            app.set_activation_policy(cap_desktop_runtime::ActivationPolicy::Accessory)
        {
...
#[cfg(target_os = "macos")]
pub(crate) fn sync_macos_dock_visibility(app: &cap_desktop_runtime::AppHandle) {
...
#[cfg(target_os = "macos")]
pub(crate) fn schedule_macos_dock_visibility_sync(app: &cap_desktop_runtime::AppHandle) {
...
fn macos_spawn_sck_displays_validation() {
    ...
    cap_desktop_runtime::async_runtime::spawn_blocking(|| {
...
fn macos_validate_sck_displays() -> bool {
    ...
    *last_attempt = Some(std::time::Instant::now());
    ...
    let validated = objc2::rc::autoreleasepool(|_| {
        cap_desktop_runtime::async_runtime::block_on(async {
...
#[cfg(target_os = "macos")]
fn macos_request_permission(app: &cap_desktop_runtime::AppHandle, permission: &OSPermission) {
    match permission {
        OSPermission::ScreenRecording => {
...
#[cfg(target_os = "macos")]
fn macos_open_permission_settings(app: &cap_desktop_runtime::AppHandle, permission: &OSPermission) {
...
#[specta::specta]
pub fn open_permission_settings(_app: cap_desktop_runtime::AppHandle, _permission: OSPermission) {
    #[cfg(target_os = "macos")]
    {
...
pub async fn request_permission(_app: cap_desktop_runtime::AppHandle, _permission: OSPermission) {
    #[cfg(target_os = "macos")]
    {
        macos_activate_permission_request(&_app);
        ...
        let app = _app.clone();
        cap_desktop_runtime::async_runtime::spawn_blocking(move || {
