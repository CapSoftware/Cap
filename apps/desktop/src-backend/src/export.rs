...
#[derive(Clone)]
struct ExportProgress(cap_desktop_runtime::ipc::Channel<FramesRendered>);
...
pub fn cancel_current_window_exports(window: cap_desktop_runtime::Window) {
...
#[cap_desktop_runtime::command]
#[specta::specta]
pub fn begin_export_session(window: cap_desktop_runtime::Window) {
    retain_export_session_for_window(window.label());
}

#[cap_desktop_runtime::command]
#[specta::specta]
pub fn end_export_session(window: cap_desktop_runtime::Window) {
...
fn resolve_exporter_binary() -> Result<PathBuf, String> {
    ...
    Err(format!(
        "Export worker binary not found; place {} next to the app executable or build the Electron sidecar bundle",
        exporter_bin_name()
    ))
}

/// Dev-loop trap: the frontend dev server rebuilds the app but not the exporter sidecar,
...
pub async fn export_video(
    ...
    progress: cap_desktop_runtime::ipc::Channel<FramesRendered>,
    settings: ExportSettings,
...
async fn show_export_save_dialog(
    ...
    app.native_request::<Option<String>, _>(
        "dialog.save",
        serde_json::json!({
            "title": "Save File",
