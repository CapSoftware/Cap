...
use cap_desktop_runtime::{AppHandle, Manager, path::BaseDirectory};
use cap_fail::fail;
...
#[cap_desktop_runtime::command(async)]
#[specta::specta]
pub async fn list_capture_windows(window: cap_desktop_runtime::Window) -> Vec<CaptureWindow> {
    ...
    F: std::future::Future<Output = Result<T, String>>,
{
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        if tokio::runtime::Handle::try_current().is_ok() {
            tokio::task::block_in_place(|| cap_desktop_runtime::async_runtime::block_on(future))
        } else {
            cap_desktop_runtime::async_runtime::block_on(future)
        }
    }));
...
#[derive(cap_desktop_runtime::Event, specta::Type, Clone, Debug, serde::Serialize)]
#[serde(tag = "variant")]
