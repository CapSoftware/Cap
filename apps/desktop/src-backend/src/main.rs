...
fn main() {
    #[cfg(target_os = "macos")]
    configure_macos_sidecar_activation_policy();
...
#[cfg(target_os = "macos")]
fn configure_macos_sidecar_activation_policy() {
    use cocoa::appkit::{
        NSApp, NSApplication,
        NSApplicationActivationPolicy::NSApplicationActivationPolicyProhibited,
    };

    // Electron owns Cap's windows, menu bar, and Dock presence. Some of the native
    // capture crates initialize AppKit in this process, which otherwise makes macOS
    // register the Rust sidecar as a second foreground copy of Cap.
    unsafe {
