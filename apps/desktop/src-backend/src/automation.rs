...
use cap_desktop_runtime::{AppHandle, Manager, Wry};
...
impl AutomationHost for DesktopAutomationHost {
    ...
    async fn upload(
        ...
        let link = if let Some(image_path) = ctx.image_path.as_ref() {
            ...
            let channel = cap_desktop_runtime::ipc::Channel::new(|_| Ok(()));
    ...
    async fn open_file(&self, ctx: &TriggerContext) -> Result<(), String> {
        ...
        self.app
            .native_operation("opener.openPath", serde_json::json!({ "path": path_str }))?;
