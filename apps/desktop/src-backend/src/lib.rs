...
use cap_desktop_runtime::Listener;
use cap_desktop_runtime::{
    AppHandle, Emitter, Manager, PhysicalPosition, State, Window, WindowEvent, ipc::Channel,
};
...
) -> cap_desktop_runtime::Result<Menu<cap_desktop_runtime::Wry>> {
...
pub(crate) fn schedule_resume_recovery(app_handle: AppHandle) {
    cap_desktop_runtime::async_runtime::spawn(async move {
        if app_is_exiting(&app_handle) {
            return;
        }
...
#[derive(specta::Type, Serialize, cap_desktop_runtime::Event, Clone)]
pub struct RecordingOptionsChanged;

#[derive(Deserialize, specta::Type, Serialize, cap_desktop_runtime::Event, Debug, Clone)]
...
#[derive(Deserialize, specta::Type, cap_desktop_runtime::Event, Debug, Clone, Serialize)]
pub struct RecordingDeleted {
    path: PathBuf,
}

#[derive(specta::Type, cap_desktop_runtime::Event, Serialize)]
...
#[derive(Serialize, Type, cap_desktop_runtime::Event, Clone)]
...
#[derive(Deserialize, specta::Type, cap_desktop_runtime::Event, Debug, Clone)]
...
#[derive(Serialize, specta::Type, cap_desktop_runtime::Event, Debug, Clone)]
...
#[derive(Serialize, specta::Type, cap_desktop_runtime::Event, Debug, Clone, PartialEq)]
...
fn focus_captures_panel(_app: AppHandle) {
    if let Some(window) = CapWindowId::RecordingsOverlay.get(&_app) {
...
#[specta::specta]
#[instrument(skip(app))]
fn open_external_link(app: cap_desktop_runtime::AppHandle, url: String) -> Result<(), String> {
    if let Ok(Some(settings)) = GeneralSettingsStore::get(&app)
    ...
    app.native_operation("shell.open", serde_json::json!({ "path": url }))?;
    Ok(())
}
...
async fn editor_delete_project(
    app: cap_desktop_runtime::AppHandle,
    editor_instance: WindowEditorInstance,
    window: cap_desktop_runtime::Window,
) -> Result<(), String> {
    let _ = window.close();
...
async fn check_notification_permissions(app: AppHandle) {
    ...
    match app
        .native_request::<String, _>("notification.permission", serde_json::json!({}))
        .await
    {
        Ok(state) if state != "granted" => {
            println!("Requesting notification permission");
            match app
                .native_request::<String, _>(
...
pub async fn open_target_picker(
    ...
    use cap_desktop_runtime::Manager;
...
#[cfg_attr(mobile, cap_desktop_runtime::mobile_entry_point)]
pub async fn run(recording_logging_handle: LoggingHandle, logs_dir: PathBuf) {
    ...
    cap_desktop_runtime::async_runtime::set(tokio::runtime::Handle::current());

    let resource_dir = std::env::var_os("CAP_ELECTRON_RESOURCE_DIR")
        .map(PathBuf::from)
        .or_else(|| std::env::current_exe().ok()?.parent().map(PathBuf::from))
        .unwrap_or_else(|| PathBuf::from("."));
    let backend_options = cap_desktop_runtime::BackendOptions::new("so.cap.desktop", resource_dir);

    if let Err(error) = cap_desktop_runtime::run_backend(
        backend_options,
        move |app| {
            let app = app.clone();
            ...
            app.manage(updates::UpdatesState);
            ...
            app.listen("electron://open-urls", move |event| {
                let Ok(urls) = serde_json::from_str::<Vec<String>>(event.payload()) else {
                    return;
                };
                let urls = urls
                    .into_iter()
                    .filter_map(|url| cap_desktop_runtime::Url::parse(&url).ok())
                    .collect();
                deeplink_actions::handle(&app_handle, urls);
            });

            let app_handle = app.clone();
            app.listen("electron://reopen", move |_| {
                handle_electron_reopen(&app_handle);
            });
                    ...
                    if let Some(window_id) = window_id {
                        ...
                        match window_id {
                            ...
                            CapWindowId::TargetSelectOverlay { display_id } => {
                                ...
                                {
                                    focus_manager.destroy(&display_id, app);
                                }
                                let session_id = app
                                    .try_state::<Arc<AtomicU64>>()
                                    .map(|state| state.load(Ordering::Acquire))
            ...
            if let Err(panic) = result {
                ...
                tracing::error!(panic = %message, "Suppressed panic in Electron WindowEvent handler");
                sentry::capture_message(
                    &format!("Electron WindowEvent panic suppressed: {message}"),
                    sentry::Level::Error,
                );
            }
        });
        ...
        |app| async move {
            let should_cleanup = app
                ...
                .is_none_or(|exit_state| exit_state.begin());
            if should_cleanup {
                if tokio::time::timeout(
                    APP_EXIT_TOTAL_TIMEOUT,
                    cleanup_app_resources_for_exit(&app),
                )
                .await
                .is_err()
                {
                    error!(
                        timeout_ms = APP_EXIT_TOTAL_TIMEOUT.as_millis(),
                        "Timed out while cleaning up the Electron backend"
                    );
                } else {
                    crash_sentinel::mark_clean_exit();
                }
    ...
    .await
    {
        error!(%error, "Electron backend stopped unexpectedly");
    }
}
...
// Catches panics inside an async desktop command body so a single bad code path can't
    ...
    match AssertUnwindSafe(fut).catch_unwind().await {
        ...
        Err(panic) => {
            ...
            error!(
                ...
                panic = %message,
                "Suppressed panic in desktop command"
            );
            sentry::capture_message(
                &format!("Desktop command '{command_name}' panicked: {message}"),
                sentry::Level::Error,
            );
...
fn handle_electron_reopen(app: &AppHandle) {
    let should_focus_onboarding = should_show_onboarding(app);
    if should_focus_onboarding && let Some(onboarding) = CapWindowId::Onboarding.get(app) {
        onboarding.show().ok();
...
#[cfg(any())]
fn handle_run_event(_handle: &AppHandle, event: cap_desktop_runtime::RunEvent) {
    match event {
        #[cfg(target_os = "macos")]
        cap_desktop_runtime::RunEvent::Reopen { .. } => {
        ...
        cap_desktop_runtime::RunEvent::ExitRequested { code, api, .. } => {
        ...
        cap_desktop_runtime::RunEvent::Exit => {
            #[cfg(target_os = "macos")]
            {
...
fn global_message_dialog(app: AppHandle, message: String) {
    let _ = app.native_operation("dialog.message", serde_json::json!({ "message": message }));
}
...
trait EventExt: cap_desktop_runtime::Event {
    fn listen_any_spawn<Fut>(
        app: &AppHandle,
        handler: impl Fn(Self, AppHandle) -> Fut + Send + Sync + 'static + Clone,
    ) -> cap_desktop_runtime::EventId
    where
        Fut: Future + Send,
...
impl<T: cap_desktop_runtime::Event> EventExt for T {}
    ...
    match &meta.inner {
        ...
        RecordingMetaInner::Instant(_) => {
            ...
            if mp4_path.exists() && mp4_path.is_file() {
                let _ = app
                    .native_operation("opener.openPath", serde_json::json!({ "path": mp4_path }));
                if let Some(main_window) = CapWindowId::Main.get(&app) {
                    main_window.hide().ok();
                }
            }
