...
use cap_desktop_runtime::{AppHandle, Manager, WebviewWindow};
use scap_targets::{
...
#[derive(cap_desktop_runtime::Event, Serialize, Type, Clone)]
...
pub async fn open_target_select_overlays(
    app: AppHandle,
    state: cap_desktop_runtime::State<'_, WindowFocusManager>,
...
pub async fn update_camera_overlay_bounds(
    ...
    window
        .set_size(cap_desktop_runtime::Size::Physical(
            cap_desktop_runtime::PhysicalSize {
                width: width_u32,
                height: height_u32,
            },
        ))
    ...
    window
        .set_position(cap_desktop_runtime::Position::Physical(
            cap_desktop_runtime::PhysicalPosition {
                x: x as i32,
                y: y as i32,
            },
        ))
...
pub async fn close_target_select_overlays(
    app: AppHandle,
    _state: cap_desktop_runtime::State<'_, WindowFocusManager>,
) -> Result<(), String> {
...
impl WindowFocusManager {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if main_window_was_seen && !main_window_available && !settings_window_available {
                    hide_overlay(&window);
                    app.state::<WindowFocusManager>().finish(&display_id, app);
                    break;
                }
    ...
    fn finish(&self, id: &DisplayId, app: &AppHandle) {
        let mut tasks = self.tasks.lock().unwrap_or_else(PoisonError::into_inner);
        tasks.remove(&id.to_string());
        drop(tasks);

        self.finish_if_idle(app);
    }

    fn finish_if_idle(&self, app: &AppHandle) {
    ...
    pub fn destroy(&self, id: &DisplayId, app: &AppHandle) {
        let mut tasks = self.tasks.lock().unwrap_or_else(PoisonError::into_inner);
    ...
    fn register_escape(&self, app: &AppHandle) {
        ...
        match app.native_operation(
            "hotkeys.escapeEnabled",
    ...
    fn unregister_escape(&self, app: &AppHandle) {
        ...
        if let Err(err) = app.native_operation(
            "hotkeys.escapeEnabled",
