...
#[cfg(not(all(target_os = "macos", target_arch = "x86_64")))]
type ParakeetReader = hound::WavReader<std::io::BufReader<std::fs::File>>;
...
impl ParakeetAudio {
    ...
    fn int_scale(&self) -> f32 {
        1.0 / (1i64 << (self.bits_per_sample.clamp(1, 32) - 1)) as f32
    }
}
...
#[cfg(not(all(target_os = "macos", target_arch = "x86_64")))]
fn open_wav_for_parakeet(
    audio_path: &std::path::Path,
) -> Result<(ParakeetReader, ParakeetAudio), String> {
    ...
    Ok((
        ...
        ParakeetAudio {
            ...
            float_samples: matches!(spec.sample_format, hound::SampleFormat::Float),
            frame_count,
        },
    ))
}
...
#[cfg(not(all(target_os = "macos", target_arch = "x86_64")))]
fn read_wav_window(
    reader: &mut ParakeetReader,
    audio: &ParakeetAudio,
    chunk: &ParakeetChunk,
) -> Result<Vec<f32>, String> {
    let start = u32::try_from(chunk.start_frame)
        .map_err(|_| "Recording is too long to index as WAV frames".to_string())?;
    reader
        .seek(start)
        .map_err(|e| format!("Failed to seek audio to frame {start}: {e}"))?;
...
fn process_with_parakeet(
    ...
    let (mut reader, audio) = open_wav_for_parakeet(audio_path)?;
    ...
    let mut collected: Vec<CaptionWord> = Vec::new();
    {
        ...
        for (index, chunk) in chunks.iter().enumerate() {
            ...
            let samples = read_wav_window(&mut reader, &audio, chunk)?;
