...
async fn extract_audio_from_video(video_path: &str, output_path: &PathBuf) -> Result<(), String> {
    ...
    let status = Command::new("ffmpeg")
        .args([
            ...
            "-acodec",
            "pcm_s16le", // PCM 16-bit little-endian audio codec
            "-ar",
            "16000", // Set audio sampling rate to 16kHz (required by Whisper)
            "-ac",
...
pub async fn save_captions(
    ...
    let project_captions = cap_project::CaptionsData {
        segments: captions
            .segments
            .iter()
            .map(|seg| cap_project::CaptionSegment {
                id: seg.id.clone(),
    ...
    match serde_json::from_str::<cap_project::CaptionsData>(&json) {
        Ok(project_captions) => {
            ...
            let tauri_captions = CaptionData {
                segments: project_captions
                    .segments
                    .iter()
                    .map(|seg| CaptionSegment {
                        id: seg.id.clone(),
