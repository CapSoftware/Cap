...
pub async fn upload_file(
    ...
    if let Some(ref options) = options {
        ...
        let file_key = format!(
            "{}/{}/{}/{}",
            options.user_id, options.video_id, file_type, file_name
        );
...
async fn get_video_duration(file_path: &str) -> Result<f64, String> {
    ...
    let hours: f64 = duration_parts[0]
        .parse()
        .map_err(|_| "Failed to parse hours")?;
    let minutes: f64 = duration_parts[1]
        .parse()
        .map_err(|_| "Failed to parse minutes")?;
    let seconds: f64 = duration_parts[2]
        .parse()
        .map_err(|_| "Failed to parse seconds")?;
