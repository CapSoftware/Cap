...
mod tests {
    ...
    #[test]
    fn camera_resolution_accepts_case_insensitive_identifiers() {
        let cameras = vec![CameraIdentity {
            display_name: "FaceTime HD Camera".to_string(),
            device_id: "Known-Device".to_string(),
            model_id: Some("1A2B:3C4D".to_string()),
        ...
        assert_eq!(
            resolve_camera_selector(
                &CameraSelector::DeviceId("known-device".to_string()),
                &cameras,
            )
            .unwrap(),
            DeviceOrModelID::DeviceID("known-device".to_string())
        );
        assert_eq!(
            resolve_camera_selector(
                &CameraSelector::Label("facetime hd camera".to_string()),
                &cameras,
            )
            .unwrap(),
            DeviceOrModelID::ModelID(
                cap_camera::ModelID::try_from("1A2B:3C4D".to_string()).unwrap()
            )
        );
    }
}
