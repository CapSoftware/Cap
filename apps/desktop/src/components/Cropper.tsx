...
export default function Cropper(
	...
	function handleResizeWithAspectRatio(
		...
		const boundaryX = newX;
		const boundaryY = newY;
	...
	return (
		...
		>
			...
			{/* Occluder */}
			<div
				class="absolute inset-0 *:absolute *:bg-black/50 *:pointer-events-none"
				aria-hidden="true"
			>
				{/* Top blind */}
			...
			<div class="size-full">
					...
					<For each={HANDLES}>
						{(handle) =>
							handle.isCorner ? (
								<button
									type="button"
									class="fixed z-50 flex h-[30px] w-[30px] *:border-white focus:outline-none focus:ring-0"
									tabIndex={-1}
				...
				{/* Full overlay to start a new selection when clicking outside the region */}
				<button
					type="button"
