
import { Component, ComponentProps } from "solid-js";
import { cx } from "cva";


function TargetTypeButton(
  props: {
    selected: boolean;
    Component: Component<ComponentProps<"svg">>;
    name: string;
  } & ComponentProps<"div">
) {
  return (
    <div
      {...props}
      class={cx(
        "flex-1 text-center hover:bg-gray-3 bg-gray-2 flex flex-col ring-offset-gray-1 ring-offset-2 items-center justify-end gap-2 py-2.5 rounded-lg transition-all",
        props.selected ? "bg-gray-3 text-white ring-blue-9 ring-1" : "ring-transparent ring-0"
      )}
    >
      <props.Component class={cx("size-5 transition-colors", props.selected ? "text-gray-12" : "text-gray-9")} />
      <p class="text-xs text-gray-12">{props.name}</p>
    </div>
  );
}

export default TargetTypeButton;
