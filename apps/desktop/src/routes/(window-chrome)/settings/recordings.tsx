...
export default function Recordings() {
	...
	return (
		<div class="flex relative flex-col space-y-4 w-full h-full">
			...
			>
				...
				<div
					class="flex relative flex-col flex-1 mt-4 rounded-xl custom-scroll bg-white/5"
					style={{
			...
			<div class="flex gap-5 items-center">
				...
				>
					<img
						...
						src={`${convertFileSrc(props.recording.thumbnailPath)}?t=${Date.now()}`}
						onError={() => setImageExists(false)}
					/>
				</Show>
				<div class="flex flex-col gap-2">
					<span class="text-white">{props.recording.prettyName}</span>
						...
						return (
							<>
								{/* <Show
									...
									fallback={
										<TooltipIconButton tooltipText="Reupload" onClick={() => reupload.mutate()}>
								...
								<Show when={props.recording.meta.sharing}>
									{(sharing) => (
										...
										<button
											onClick={() => shell.open(sharing().link)}
											class="flex flex-row gap-1 items-center justify-center h-8 px-2 bg-white/[0.03] hover:bg-white/5 border border-white/5 rounded-[8px] text-white"
										>
											<p class="text-md">View</p>
											<ArrowUpRight class="size-4 text-white/70" />
										</button>
									)}
								</Show>
							</>
