    ...
    <Switch>
      ...
      <Match when={rawOptions.targetMode === "area"}>
        {(_) => {
          ...
          const [placeControlsAbove, setPlaceControlsAbove] = createSignal(false);
...
function RecordingControls(props: { target: ScreenCaptureTarget }) {
  ...
  return (
    ...
    >
      ...
      <p class="text-sm text-white text-nowrap"><span class="font-medium">Start Recording</span>:</p>
      <div
      onClick={(e) => {
        e.stopPropagation();
      ...
      <p class="pl-0.5 text-sm text-nowrap text-white">{capitalize(rawOptions.mode) + " Mode"}</p>
