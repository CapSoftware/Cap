...
import { type SceneSegment, type SplitViewSettings } from "~/utils/tauri";
...
function SimplePositionControl(props: {
	...
	return (
		...
		>
			...
			{/* Label */}
			<div class="absolute bottom-1 left-1 px-1.5 py-0.5 bg-gray-1/80 rounded text-xs text-gray-11 pointer-events-none">
				{props.label}
			</div>
			
			{/* Coordinates */}
			<div class="absolute top-1 right-1 px-1.5 py-0.5 bg-gray-1/80 rounded text-xs text-gray-11 pointer-events-none">
						...
						{(option) => (
							<KRadioGroup.Item value={option.value} class="relative">
								...
								<KRadioGroup.ItemControl class="flex flex-col gap-1 p-3 w-full rounded-lg border border-gray-3 bg-gray-1 ui-checked:bg-gray-3 ui-checked:border-blue-9 transition-all cursor-pointer hover:border-gray-5 ui-checked:shadow-sm">
									<div class="flex items-center gap-2">
										{option.icon}
										<span class="text-sm font-medium text-gray-12">{option.label}</span>
									</div>
			...
			<Show when={props.segment.mode === "splitView"}>
				<div class="space-y-3 pt-3 border-t border-gray-3">
					<div class="flex items-center gap-2">
						...
						<span class="text-sm font-medium text-gray-12">Split View Settings</span>
					</div>
					
					<div class="space-y-4">
						<div class="space-y-2">
							<label class="text-xs font-medium text-gray-11">Camera Side</label>
							<KRadioGroup
						...
						<div class="grid grid-cols-2 gap-3">
							<div class="space-y-3">
								<div class="space-y-2">
									<label class="text-xs font-medium text-gray-11">Camera Position</label>
								...
								<div class="space-y-2">
									<div class="flex justify-between items-center">
										<label class="text-xs font-medium text-gray-11">Camera Zoom</label>
										<span class="text-xs text-gray-10">{((splitViewSettings().cameraZoom || 1) * 100).toFixed(0)}%</span>
									</div>
									<Slider
							...
							<div class="space-y-3">
								<div class="space-y-2">
									<label class="text-xs font-medium text-gray-11">Screen Position</label>
									<SimplePositionControl
								...
								<div class="space-y-2">
									<div class="flex justify-between items-center">
										<label class="text-xs font-medium text-gray-11">Screen Zoom</label>
										<span class="text-xs text-gray-10">{((splitViewSettings().screenZoom || 1) * 100).toFixed(0)}%</span>
									</div>
									<Slider
