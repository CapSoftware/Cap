...
export function VideoOverlay(props: {
	...
	const selected = (index: number) =>
		editorState.timeline.selection?.type === "video" &&
	...
	function drag(
		...
		endDrag?.();
		const source = project.timeline?.videoSegments[index];
		if (!source) return;
		...
		const targets = snapTargets({ video: index });
		const resume = projectHistory.pause();
		let moved = false;
		let frame: number | undefined;
		let pending: MouseEvent | undefined;
		const move = (next: MouseEvent) => {
			if (project.timeline?.videoSegments[index] !== source) return;
			const delta = {
			...
			if (mode === "rotate") {
				...
				setProject("timeline", "videoSegments", index, "center", {
		...
		const finish = (next?: MouseEvent, cancelled = false) => {
			...
			if (
				cancelled &&
				moved &&
				project.timeline?.videoSegments[index] === source
			)
			...
			<Show when={!editorState.playing}>
				<For each={visible()}>
					{({ segment, index }) => (
						<div
							data-video-overlay
							class="group/video absolute pointer-events-auto cursor-move"
							style={{
								"z-index": getOverlayZIndex(project, "video", segment.track),
						...
						>
							<div
								class={`pointer-events-none absolute inset-0 border-2 ${selected(index) ? "border-blue-9" : "border-transparent group-hover/video:border-blue-7"}`}
							/>
