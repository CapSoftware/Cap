...
function ShareButton() {
    ...
    mutationFn: async () => {
      ...
      if (!canShare.allowed) {
        if (canShare.reason === "upgrade_required") {
          ...
          throw new Error("Upgrade required to share recordings longer than 5 minutes");
        }
      }
      ...
      const result = meta().sharing
        ? await commands.uploadExportedVideo(projectPath, "Reupload", uploadChannel, null, null)
  ...
  createProgressBar(() => {
    ...
    if (uploadState.type === "rendering") return (uploadState.renderedFrames / uploadState.totalFrames) * 100;
        ...
        {(sharing) => {
          ...
          const customUrl = () =>
            customDomain.data?.custom_domain
              ? new URL(customDomain.data?.custom_domain + `/s/${meta().sharing?.id}`)
              : null;
          ...
          const [linkToDisplay, setLinkToDisplay] = createSignal<string | null>(
            customDomain.data?.custom_domain && customDomain.data?.domain_verified ? customLink : normalLink
          );
          ...
          return (
            <div class="flex gap-3 items-center">
              <Tooltip content={upload.isPending ? "Reuploading video" : "Reupload video"}>
                <Button
              ...
              <Tooltip content="Open link">
                <div class="rounded-xl px-3 py-2 flex flex-row items-center gap-[0.375rem] bg-gray-3 hover:bg-gray-4 transition-colors duration-100">
                  <a
                    href={linkToDisplay() === customLink ? copyLinks.custom : copyLinks.normal}
                    target="_blank"
                    rel="noreferrer"
                  ...
                  {/** Dropdown */}
                  <Show when={customDomain.data?.custom_domain && customDomain.data?.domain_verified}>
                      ...
                      <KSelect
                        ...
                        itemComponent={(props) => (
                          <MenuItem<typeof KSelect.Item> as={KSelect.Item} item={props.item}>
                            <KSelect.ItemLabel class="flex-1 text-xs truncate">{props.item.rawValue}</KSelect.ItemLabel>
                          </MenuItem>
                        )}
                      ...
                      >
                        ...
                        <KSelect.Portal>
                          <PopperContent<typeof KSelect.Content> as={KSelect.Content} class={topLeftAnimateClasses}>
                            <MenuItemList<typeof KSelect.Listbox> as={KSelect.Listbox} class="w-[236px]" />
                          </PopperContent>
                        </KSelect.Portal>
                      </KSelect>
                    </Tooltip>
                ...
                style={{
                  width: `${
                    uploadState.type === "uploading"
                      ...
                      ? Math.min((uploadState.renderedFrames / uploadState.totalFrames) * 100, 100)
                      : 0
                  }%`,
                }}
              />
