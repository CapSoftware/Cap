...
export type BlurSegmentDragState =
    | { type: "idle" }
    | { type: "movePending" }
    | { type: "moving" };

export function BlurTrack(props: {
    onDragStateChanged: (v: BlurSegmentDragState) => void;
    ...
    return (
            ...
            onMouseDown={(e) => {
                createRoot((dispose) => {
                    createEventListener(e.currentTarget, "mouseup", (e) => {
                        ...
                        e.stopPropagation();
                        batch(() => {
                            setProject("timeline", "blurSegments", (v) => v ?? []);
                            setProject(
                                "timeline",
                                "blurSegments",
                                produce((blurSegments) => {
                                    blurSegments ??= [];
        ...
        >
            <For
                each={project.timeline?.blurSegments}
                fallback={
            ...
            >
                {(segment, i) => {
                    ...
                    const isSelected = createMemo(() => {
                        ...
                        if (!selection || selection.type !== "blur") return false;

                        const segmentIndex = project.timeline?.blurSegments?.findIndex(
                    ...
                    return (
                        ...
                        class={cx(
                          ...
                          "bg-blue-500/30 backdrop-blur-md shadow-[inset_0_4px_8px_rgba(0,0,0,0.2)]",
                          isSelected()
                            ...
                            <SegmentContent
                                ...
                                onMouseDown={createMouseDownDrag(
                                    () => {
                                        ...
                                        const prevSegment = blurSegments()[i() - 1];
                                        const nextSegment = blurSegments()[i() + 1];
                            ...
                            <SegmentHandle
                                ...
                                onMouseDown={createMouseDownDrag(
                                    () => {
                                        ...
                                        for (let i = 0; i < blurSegments().length; i++) {
                                            const segment = blurSegments()[i]!;
