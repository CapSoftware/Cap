...
export function ExportDialog() {
  ...
  const exportEstimates = createQuery(() => ({
    ...
    queryFn: ({ queryKey: [_, { resolution, fps }] }) => commands.getExportEstimates(projectPath, resolution, fps),
  }));
  ...
  const upload = createMutation(() => ({
    mutationFn: async () => {
      ...
      await exportWithSettings((progress) => setExportState({ type: "rendering", progress }));
            ...
            <div class="overflow-hidden relative p-4 rounded-xl dark:bg-gray-2 bg-gray-3">
              <div class="flex flex-col gap-3">
                ...
                <KSelect<{ label: string; value: number }>
                  ...
                  onChange={(option) => {
                    const value = option?.value ?? (settings.format === "Gif" ? 10 : 30);
                      ...
                      {(copyState) => (
                        <div class="flex flex-col gap-4 justify-center items-center h-full">
                          ...
                          <Show
                            ...
                            keyed
                          >
                            {(copyState) => <RenderProgress state={copyState} format={settings.format} />}
                          </Show>
                        </div>
                      )}
                    </Match>
                            ...
                            {(uploadState) => (
                              <div class="flex flex-col gap-4 justify-center items-center">
                                ...
                                <Switch>
                                  ...
                                  <Match when={uploadState.type !== "uploading" && uploadState} keyed>
                                    {(renderState) => <RenderProgress state={renderState} format={settings.format} />}
                                  </Match>
                                </Switch>
                              </div>
                            )}
