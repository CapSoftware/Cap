...
import { Box, type Ratio, CropBounds } from "~/utils/cropController";
...
export default function Cropper(
  ...
  createEffect(() => {
    if (!selAreaRef) return;
    const rawBounds = roundBounds(controller._rawBounds());
    console.log(`rawBounds: ${JSON.stringify(rawBounds)}`);

    selAreaRef.style.top = `${rawBounds.y}px`;
    selAreaRef.style.left = `${rawBounds.x}px`;
    selAreaRef.style.width = `${rawBounds.width}px`;
    selAreaRef.style.height = `${rawBounds.height}px`;
  ...
  createEffect(() => {
    ...
    const rawBounds = controller._rawBounds();
    snapEl.style.top = `${rawBounds.y + 10}px`;
    snapEl.style.left = `${
      rawBounds.x + rawBounds.width / 2 - snapRatioElementWidthPx() / 2
    }px`;
  });
  ...
  function handleDragStart(e: MouseEvent) {
    ...
    createRoot((dispose) => {
      const initialBox = controller._getRawBox();
      const container = controller.containerSize();
      const initialMousePos = {
        x: e.clientX - initialBox.x,
        y: e.clientY - initialBox.y,
      };
  ...
  function handleResizeStartTouch(dir: Direction, event: TouchEvent) {
  ...
  return (
    ...
    >
      ...
      <div
        ref={selAreaRef}
        class="absolute border-red-10 border"
        style={
          {
          ...
          {(handle) => {
            ...
            return isCorner ? (
              <div
                ...
                onTouchStart={[handleResizeStartTouch, handle.direction]}
              />
            ) : (
              <div
                role="slider"
          ...
          >
            <div
              ...
              style={{
                ...
                top: `${controller._rawBounds().y + 10}px`,
                left: `${
                  controller._rawBounds().x +
                  controller._rawBounds().width / 2 -
                  snapRatioElementWidthPx() / 2
                }px`,
              }}
              class="absolute bg-gray-3 opacity-90 h-6 rounded-[7px] text-center text-blue-11 text-sm border border-blue-9/40 outline-[#dedede] dark:outline-[#000]"
            >
            ...
            {(ratio) => {
              return (
                <div
                  ...
                  class="absolute bg-gray-3 opacity-80 h-6 rounded-[7px] text-center text-neutral-300 text-sm border border-neutral-50/20 outline-[#dedede] dark:outline-[#000] flex items-center justify-evenly"
                >
                  <Show
                    when={
