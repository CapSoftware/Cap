...
async getLogsAndSystemInfo() : Promise<LogsAndSystemInfo> {
    return await TAURI_INVOKE("get_logs_and_system_info");
},
async getLogFiles(paths: string[]) : Promise<LogFile[]> {
    return await TAURI_INVOKE("get_log_files", { paths });
},
async getLastRecordingZip() : Promise<LastRecording | null> {
    return await TAURI_INVOKE("get_last_recording_zip");
},
async getRecordingZip(recordingPath: string | null) : Promise<LastRecording | null> {
    return await TAURI_INVOKE("get_recording_zip", { recordingPath });
}
}
...
export type DisplayInfo = { width: number; height: number; scale_factor: number }
...
export type GeneralSettingsStore = { instanceId?: string; uploadIndividualFiles?: boolean; hideDockIcon?: boolean; hapticsEnabled?: boolean; autoCreateShareableLink?: boolean; enableNotifications?: boolean; disableAutoOpenLinks?: boolean; hasCompletedStartup?: boolean; theme?: AppTheme; commercialLicense?: CommercialLicense | null; lastVersion?: string | null; windowTransparency?: boolean; postStudioRecordingBehaviour?: PostStudioRecordingBehaviour; mainWindowRecordingStartBehaviour?: MainWindowRecordingStartBehaviour; custom_cursor_capture2?: boolean; serverUrl?: string; recordingCountdown?: number | null; enableNativeCameraPreview: boolean; autoZoomOnClicks?: boolean; enableNewRecordingFlow: boolean; postDeletionBehaviour?: PostDeletionBehaviour }
...
export type LogsAndSystemInfo = { system_info: SystemInfo; recent_logs: RecordingLog[]; app_version: string }
...
export type RecordingLog = { id: string; timestamp: string; duration_seconds: number | null; error: string | null; log_content: string | null; log_file_path: string | null }
...
export type SystemInfo = { os: string; os_version: string; arch: string; cpu_cores: number; memory_gb: number; displays: DisplayInfo[]; cameras: string[]; microphones: string[] }
export type TargetUnderCursor = { display_id: DisplayId | null; window: WindowUnderCursor | null; screen: ScreenUnderCursor | null }
export type TimelineConfiguration = { segments: TimelineSegment[]; zoomSegments: ZoomSegment[]; layoutSegments?: LayoutSegment[] }
