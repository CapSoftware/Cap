...
export class Store {
	...
	constructor(
		...
		this.loaded = native<Record<string, unknown>>("store.load", { path }).then(
			(values) => {
	...
	async save() {
		await this.loaded;
		return native<void>("store.save", {
			path: this.path,
			value: Object.fromEntries(this.data),
		});
	}
	async reload() {
		const values = await native<Record<string, unknown>>("store.load", {
			path: this.path,
		});
		this.data = new Map(Object.entries(values));
	}
	...
	async onChange<T>(_callback: (key: string, value: T | undefined) => void) {
		return () => {};
	}
}
