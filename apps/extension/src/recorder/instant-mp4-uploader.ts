...
const postJson = async <TResponse>(
	...
	options?: { apiOrigin?: string; apiKey?: string },
): Promise<TResponse> => {
	const origin =
		options?.apiOrigin && options.apiOrigin.trim().length > 0
			? options.apiOrigin
			: typeof window !== "undefined"
				? window.location.origin
				: "";
	const requestUrl = origin ? new URL(url, origin).toString() : url;
	...
	const response = await fetch(requestUrl, {
		...
		credentials: options?.apiKey ? "omit" : "same-origin",
	...
	if (!response.ok) {
		...
		throw new Error(
			`Request to ${requestUrl} failed: ${response.status} ${message}`,
		);
	}
...
export class InstantMp4Uploader {
	...
	private readonly apiOrigin: string | undefined;
	...
	private async uploadPart(partNumber: number, part: Blob) {
		const presignedUrl = await presignMultipartPart(
			...
			partNumber,
			this.subpath,
			{ apiOrigin: this.apiOrigin, apiKey: this.apiKey },
		);
