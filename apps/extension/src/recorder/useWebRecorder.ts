...
import { useEffectMutation, useRpcClient } from "@/lib/effect-hooks";
import {
	InstantMp4Uploader,
	initiateMultipartUpload,
} from "./instant-mp4-uploader";
import type { RecordingMode } from "./RecordingModeSelector";
import { getScreenshotUploadUrl, resetVideoResultFile } from "./recorder-api";
...
export const useWebRecorder = ({
	...
	>[0];
	const setUploadStatus = useCallback((_: UploadStatus | undefined) => {}, []);
	...
	const startRecording = async (options?: { reuseInstantVideo?: boolean }) => {
		...
		try {
			...
			const handleVideoEnded = () => {
				stopRecordingRef.current?.().catch((error) => {
					console.error("Failed to stop recording after stream ended", error);
				});
			};
	...
	const stopRecording = useCallback(async () => {
		...
		try {
			...
			if (useInstantMp4) {
				processedRecordingBlob =
					rawRecordingBlob.type === "video/mp4"
						? rawRecordingBlob
						: new File([rawRecordingBlob], "result.mp4", { type: "video/mp4" });
			} else {
			...
			if (thumbnailBlob) {
				try {
					const screenshotUpload = await getScreenshotUploadUrl({
						apiOrigin,
						apiKey,
						videoId: creationResult.id,
					});
					...
					Object.entries(screenshotUpload.fields).forEach(([key, value]) => {
						screenshotFormData.append(key, value);
					});
					...
					await new Promise<void>((resolve, reject) => {
						...
						xhr.open("POST", screenshotUpload.url);
	...
	const restartRecording = useCallback(async () => {
		...
		try {
			...
			if (shouldReuseInstantVideo && creationToReuse) {
				await resetVideoResultFile({
					apiOrigin,
					apiKey,
					videoId: creationToReuse.id,
				});
			}
