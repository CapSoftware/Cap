...
export const commands = {
	...
	async setCameraInput(
		id: DeviceOrModelID | null,
		skipCameraWindow: boolean | null,
	): Promise<null> {
	...
	async generateExportPreview(
		...
		return await TAURI_INVOKE("generate_export_preview", {
			projectPath,
			frameTime,
			settings,
		});
	},
	async generateExportPreviewFast(
		frameTime: number,
		settings: ExportPreviewSettings,
	): Promise<ExportPreviewResult> {
		return await TAURI_INVOKE("generate_export_preview_fast", {
			frameTime,
			settings,
		});
	},
	...
	async updateProjectConfigInMemory(
		config: ProjectConfiguration,
		frameNumber: number | null,
		fps: number | null,
		resolutionBase: XY<number> | null,
	): Promise<null> {
		return await TAURI_INVOKE("update_project_config_in_memory", {
			config,
			frameNumber,
			fps,
			resolutionBase,
	...
	async uploadExportedVideo(
		path: string,
		mode: UploadMode,
		channel: TAURI_CHANNEL<UploadProgress>,
		organizationId: string | null,
	): Promise<UploadResult> {
		return await TAURI_INVOKE("upload_exported_video", {
			path,
			mode,
			channel,
			organizationId,
	...
	async getRecordingMeta(
		path: string,
		fileType: FileType,
	): Promise<RecordingMetaWithMetadata> {
	...
	async openTargetSelectOverlays(
		focusedTarget: ScreenCaptureTarget | null,
		specificDisplayId: string | null,
		targetMode: RecordingTargetMode | null,
	): Promise<null> {
	...
	async updateCameraOverlayBounds(
		x: number,
		y: number,
		width: number,
		height: number,
	): Promise<null> {
		return await TAURI_INVOKE("update_camera_overlay_bounds", {
			x,
			y,
			width,
			height,
	...
	async formatProjectName(
		...
		return await TAURI_INVOKE("format_project_name", {
			template,
			targetName,
			targetKind,
			recordingMode,
...
export type CameraWithFormats = {
	deviceId: string;
	displayName: string;
	modelId: string | null;
	formats: CameraFormatInfo[];
	bestFormat: CameraFormatInfo | null;
};
...
export type CaptureDisplayWithThumbnail = {
	id: DisplayId;
	name: string;
	refresh_rate: number;
...
export type CaptureWindowWithThumbnail = {
	id: WindowId;
	owner_name: string;
	name: string;
	bounds: LogicalBounds;
...
export type CurrentRecordingTarget =
	| { window: { id: WindowId; bounds: LogicalBounds | null } }
	| { screen: { id: DisplayId } }
...
export type CursorConfiguration = {
	...
	baseRotation?: number;
	clickSpring?: ClickSpringConfig | null;
	stopMovementInLastSeconds?: number | null;
};
...
export type DisplayInformation = {
	...
	logical_size: LogicalSize | null;
	logical_bounds: LogicalBounds | null;
	refresh_rate: string;
};
...
export type ExportPreviewSettings = {
	fps: number;
	resolution_base: XY<number>;
	compression_bpp: number;
};
...
export type GeneralSettingsStore = {
	...
	crashRecoveryRecording?: boolean;
	maxFps?: number;
	editorPreviewQuality?: EditorPreviewQuality;
	mainWindowPosition?: WindowPosition | null;
	cameraWindowPosition?: WindowPosition | null;
	cameraWindowPositionsByMonitorName?: { [key in string]: WindowPosition };
};
...
export type ImportStage =
	...
	| "Complete"
	| "Failed";
export type IncompleteRecordingInfo = {
	projectPath: string;
	prettyName: string;
	segmentCount: number;
	estimatedDurationSecs: number;
};
export type InstantRecordingMeta =
	| { recording: boolean }
	| { error: string }
	| { fps: number; sample_rate: number | null };
...
export type Mp4ExportSettings = {
	...
	custom_bpp: number | null;
	force_ffmpeg_decoder?: boolean;
};
...
export type MultipleSegments = {
	...
	cursors: Cursors;
	status?: StudioRecordingStatus | null;
};
...
export type RecordingEvent =
	...
	| { variant: "InputLost"; input: RecordingInputKind }
	| { variant: "InputRestored"; input: RecordingInputKind }
	| { variant: "Degraded"; reason: string }
	| { variant: "Recovered" };
...
export type RecordingMetaWithMetadata = ((
	| StudioRecordingMeta
	...
	sharing?: SharingMeta | null;
	upload?: UploadMeta | null;
}) & { mode: RecordingMode; status: StudioRecordingStatus };
export type RecordingMode = "studio" | "instant" | "screenshot";
export type RecordingOptionsChanged = null;
export type RecordingSettingsStore = {
...
export type ScreenCaptureTarget =
	...
	| { variant: "cameraOnly" };
export type ScreenMovementSpring = {
	stiffness: number;
	damping: number;
	mass: number;
};
...
export type SerializedScreenshotEditorInstance = {
	framesSocketUrl: string;
	path: string;
...
export type ShowCapWindow =
	...
	| { Camera: { centered: boolean } }
	| { InProgressRecording: { countdown: number | null } }
	| "Upgrade"
	| "ModeSelect"
	| { ScreenshotEditor: { path: string } };
...
export type SystemDiagnostics = {
	macosVersion: MacOSVersionInfo | null;
