...
const DEFAULT_HOST = process.env.TINYBIRD_HOST?.trim() || "https://api.tinybird.co";
...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i++) {
    ...
    else if (a === "--max-cities") args.maxCities = Number(argv[++i] || MAX_CITY_COUNT) || MAX_CITY_COUNT;
    else if (a === "--limit") {
...
function loadVideoToOrgMap(args, videoIds) {
  ...
  if (args.orgs.length) {
    ...
    if (args.orgs.length === 1) {
      ...
      usageAndExit("Provide either one --org for all videos or one --org per video");
    }
  }
  return map;
}
...
function resolveRegionName(region, regionCode, country) {
  ...
  const fallback = normalizeDimensionField(region) || normalizeDimensionField(regionCode);
  return fallback;
}
...
function resolveCountryCode(countryCode, country, continent) {
  ...
  const normalizedCountryCode = normalizeDimensionField(countryCode).toUpperCase();
  const normalizedCountryName = normalizeDimensionField(country);
  const upperCountryName = normalizedCountryName ? normalizedCountryName.toUpperCase() : "";
  ...
  if (normalizedCountryName) {
    for (const [code, name] of Object.entries(COUNTRIES)) {
      if (name.toUpperCase() === upperCountryName || name === normalizedCountryName) {
        if (normalizedContinent) {
...
async function dubFetch(pathname, token, params = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const base = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1000 : 500 * 2 ** (attempt - 1);
...
async function dubListLinks({ token, domain, limit = 100, offset = 0 }) {
  ...
  const links = Array.isArray(res?.links) ? res.links : Array.isArray(res) ? res : [];
...
async function dubRetrieveBreakdown({ token, domain, key, start, end, interval, timezone, groupBy, city, country, region }) {
  ...
  for (const row of rows) {
    ...
    const name = normalizeDimensionField(
      row.name ?? row.city ?? row.country ?? row.region ?? row.device ?? row.browser ?? row.os ?? row.key
    );
    if (!name) continue;
    const value = Number(row.count ?? row.clicks ?? row.value ?? row.total ?? row.n ?? 0) || 0;
...
async function migrateVideo({ tokenDub, tb, domain, videoId, orgId = "", window, limits, dryRun, apiConcurrency }) {
  ...
  const selectedCities = cities
    ...
    .map((city) => {
      const resolvedRegion = resolveRegionName(city.region, city.regionCode, city.countryCode || city.country);
  ...
  for (const city of selectedCities) {
    const resolvedCountryCode = resolveCountryCode(
      city.countryCode,
      city.country,
      city.continent || (city.countryCode ? COUNTRIES_TO_CONTINENTS[city.countryCode.toUpperCase()] : undefined)
    );
    if (resolvedCountryCode) {
      cityToCountryCodeMap.set(city.name, resolvedCountryCode);
      cityToCountryMap.set(city.name, COUNTRIES[resolvedCountryCode] || city.country || "");
  ...
  const citiesMissingCountry = selectedCities.filter((city) => !cityToCountryMap.has(city.name));
  await mapWithConcurrency(citiesMissingCountry, async (city) => {
    const cityWithCountry = await apiLimit(() => dubRetrieveBreakdown({ ...baseArgs, groupBy: "countries", city: city.name }));
    const validCountries = cityWithCountry.filter((c) => c.name && c.name !== "*" && c.value > 0);
    if (validCountries.length === 1) {
      const resolvedCountryCode = resolveCountryCode(
        validCountries[0].countryCode,
        validCountries[0].name,
        validCountries[0].continent || (validCountries[0].countryCode ? COUNTRIES_TO_CONTINENTS[validCountries[0].countryCode.toUpperCase()] : undefined)
      );
      if (resolvedCountryCode) {
        cityToCountryCodeMap.set(city.name, resolvedCountryCode);
        cityToCountryMap.set(city.name, COUNTRIES[resolvedCountryCode] || validCountries[0].name);
      } else {
      ...
      const resolvedCountryCode = resolveCountryCode(
        ...
        topCountry.name,
        topCountry.continent || (topCountry.countryCode ? COUNTRIES_TO_CONTINENTS[topCountry.countryCode.toUpperCase()] : undefined)
      );
      if (resolvedCountryCode) {
        cityToCountryCodeMap.set(city.name, resolvedCountryCode);
        cityToCountryMap.set(city.name, COUNTRIES[resolvedCountryCode] || topCountry.name);
      } else {
        cityToCountryMap.set(city.name, topCountry.name);
      }
    }
    const cityWithRegion = await apiLimit(() => dubRetrieveBreakdown({ ...baseArgs, groupBy: "regions", city: city.name }));
    const validRegions = cityWithRegion.filter((r) => r.name && r.name !== "*" && r.value > 0);
    if (validRegions.length === 1) {
      const regionCountryCode = validRegions[0].countryCode || validRegions[0].country || city.country;
      ...
      if (!cityToCountryCodeMap.has(city.name)) {
        const resolvedCountryCode = resolveCountryCode(
          ...
          validRegions[0].continent || (validRegions[0].countryCode ? COUNTRIES_TO_CONTINENTS[validRegions[0].countryCode.toUpperCase()] : undefined)
        );
        if (resolvedCountryCode) {
          cityToCountryCodeMap.set(city.name, resolvedCountryCode);
          if (!cityToCountryMap.has(city.name)) {
            cityToCountryMap.set(city.name, COUNTRIES[resolvedCountryCode] || validRegions[0].country || city.country || "");
          }
        }
      }
    } else if (validRegions.length > 1) {
      const topRegion = validRegions.sort((a, b) => b.value - a.value)[0];
      const regionCountryCode = topRegion.countryCode || topRegion.country || city.country;
      const resolvedRegion = resolveRegionName(
      ...
      if (!cityToCountryCodeMap.has(city.name)) {
        const resolvedCountryCode = resolveCountryCode(
          ...
          topRegion.continent || (topRegion.countryCode ? COUNTRIES_TO_CONTINENTS[topRegion.countryCode.toUpperCase()] : undefined)
        );
        if (resolvedCountryCode) {
          cityToCountryCodeMap.set(city.name, resolvedCountryCode);
          if (!cityToCountryMap.has(city.name)) {
            cityToCountryMap.set(city.name, COUNTRIES[resolvedCountryCode] || topRegion.country || city.country || "");
          }
        }
      }
    }
  }, apiConcurrency || DEFAULT_API_CONCURRENCY);
  const citiesMissingRegion = selectedCities.filter((city) => !cityToRegionMap.has(city.name));
  await mapWithConcurrency(citiesMissingRegion, async (city) => {
  ...
  const unresolvedCountryCities = selectedCities.filter((city) => !cityToCountryMap.has(city.name));
  if (unresolvedCountryCities.length > 0 && countries.length > 0) {
    const topCountries = countries.slice(0, 10);
    const countryCitiesList = await mapWithConcurrency(
      topCountries,
      (country) => apiLimit(() =>
        dubRetrieveBreakdown({ ...baseArgs, groupBy: "cities", country: country.name })
    ...
    for (const { country, rows: countryCities } of countryCitiesList) {
      for (const city of unresolvedCountryCities) {
        ...
        const matchingCity = countryCities.find((c) => c.name === city.name || c.city === city.name);
        if (matchingCity) {
          if (!cityToCountryCodeMap.has(city.name)) {
            const resolvedCountryCode = resolveCountryCode(
              matchingCity.countryCode,
              matchingCity.country || country.name,
              matchingCity.continent || (matchingCity.countryCode ? COUNTRIES_TO_CONTINENTS[matchingCity.countryCode.toUpperCase()] : undefined)
            );
            if (resolvedCountryCode) {
              cityToCountryCodeMap.set(city.name, resolvedCountryCode);
              cityToCountryMap.set(city.name, COUNTRIES[resolvedCountryCode] || matchingCity.country || country.name || matchingCity.name);
            } else {
              const resolvedCountry = matchingCity.country || country.name || matchingCity.name;
              if (resolvedCountry) cityToCountryMap.set(city.name, resolvedCountry);
            }
          } else if (!cityToCountryMap.has(city.name)) {
            const resolvedCountryCode = cityToCountryCodeMap.get(city.name);
            cityToCountryMap.set(city.name, COUNTRIES[resolvedCountryCode] || matchingCity.country || country.name || matchingCity.name);
          }
          if (!cityToRegionMap.has(city.name) && (matchingCity.region || matchingCity.regionCode)) {
            const resolvedRegion = resolveRegionName(
  ...
  await mapWithConcurrency(selectedCities, async (c) => {
    const series = await apiLimit(() => dubRetrieveTimeseries({ ...baseArgs, city: c.name }));
    perCityTimeseries.set(c.name, series);
  }, apiConcurrency || DEFAULT_API_CONCURRENCY);
  
  const perBrowserTimeseries = new Map();
  const topBrowsers = browsers.filter((b) => b.value > 0).sort((a, b) => b.value - a.value).slice(0, 10);
  await mapWithConcurrency(topBrowsers, async (browser) => {
    const series = await apiLimit(() => dubRetrieveTimeseries({ ...baseArgs, browser: browser.name }));
    perBrowserTimeseries.set(browser.name, series);
  }, apiConcurrency || DEFAULT_API_CONCURRENCY);
  
  const perDeviceTimeseries = new Map();
  const topDevices = devices.filter((d) => d.value > 0).sort((a, b) => b.value - a.value).slice(0, 5);
  await mapWithConcurrency(topDevices, async (device) => {
    const series = await apiLimit(() => dubRetrieveTimeseries({ ...baseArgs, device: device.name }));
    perDeviceTimeseries.set(device.name, series);
  }, apiConcurrency || DEFAULT_API_CONCURRENCY);
  
  const perOSTimeseries = new Map();
  const topOS = os.filter((o) => o.value > 0).sort((a, b) => b.value - a.value).slice(0, 5);
  await mapWithConcurrency(topOS, async (osItem) => {
    const series = await apiLimit(() => dubRetrieveTimeseries({ ...baseArgs, os: osItem.name }));
  ...
  for (const seriesItem of timeseries) {
    ...
    for (const [browserName, browserSeries] of perBrowserTimeseries.entries()) {
      const dayEntry = browserSeries.find((s) => String(s.timestamp).slice(0, 10) === day);
    ...
    for (const [deviceName, deviceSeries] of perDeviceTimeseries.entries()) {
      const dayEntry = deviceSeries.find((s) => String(s.timestamp).slice(0, 10) === day);
      if (dayEntry && dayEntry.count > 0) {
    ...
    for (const [osName, osSeries] of perOSTimeseries.entries()) {
      const dayEntry = osSeries.find((s) => String(s.timestamp).slice(0, 10) === day);
      if (dayEntry && dayEntry.count > 0) {
    ...
    for (const [cityName, citySeries] of perCityTimeseries.entries()) {
      const dayEntry = citySeries.find((s) => String(s.timestamp).slice(0, 10) === day);
      ...
      if (resolvedCountryCode && COUNTRIES[resolvedCountryCode]) {
        ...
        const fallbackCountry = cityToCountryMap.get(cityName) || cityMeta?.country || "";
        if (fallbackCountry) {
      ...
      for (const browser of dayBrowsers) {
        const browserProportion = totalBrowserClicks > 0 ? browser.count / totalBrowserClicks : 0;
      ...
      if (browserRemainder > 0 || totalBrowserClicks === 0) {
        const defaultBrowser = dayBrowsers[0]?.name || topBrowsers[0]?.name || "unknown";
        const defaultDevice = dayDevices[0]?.name || topDevices[0]?.name || "desktop";
...
async function main() {
  ...
  if (!args.dryRun && !args.apply) usageAndExit("Specify --apply to perform writes or omit to dry run");
  ...
  if (args.limit && args.limit > 0 && args.limit < videoIds.length) {
    ...
    console.log(`Limiting to ${args.limit} videos (out of ${originalCount} total)`);
  } else if (args.limit && args.limit > 0) {
    console.log(`Limit of ${args.limit} specified, but only ${originalCount} videos found`);
  }
  ...
  const totalPlanned = results.reduce((acc, r) => acc + (r.plannedEvents || 0), 0);
