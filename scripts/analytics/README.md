...
Staging also requires `TINYBIRD_STAGING_SCHEDULER_TOKEN`, a protected token with schedule-control access only to the Copy Pipes enumerated by the staging runner. The destructive erasure phase cancels and pauses those schedules before deleting rows, runs each replacement Copy exactly once, proves completion with scoped state transitions or copy-run markers, and resumes every schedule in an always-run recovery step. A failed or ambiguous Copy submission is never retried automatically.
