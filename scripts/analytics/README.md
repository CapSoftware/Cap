...
Every downstream decision Copy reads the completed `product_events_canonical_v1` generation created directly from raw deliveries at the pipeline's common source cutoff. A staging probe accepted after that cutoff must remain visible in raw health while contributing zero canonical and decision events to the active generation. The controller requires workspace-wide Copy-job quiescence before every submission, persists each accepted job ID before polling, and runs one Copy at a time. It retries only explicit provider rate-limit or Copy-quota rejections after another quiescence proof and bounded backoff; permission failures, ambiguous submission outcomes, and unresolved jobs fail closed. A shared 30-minute pipeline deadline leaves recovery time for schedule compensation and evidence upload. Recovery rebuilds and verifies zero synthetic raw, health, and decision impact before rollback; recovery records but does not enforce the normal performance gate. The 1,000-row ingestion fixture is an absolute startup-latency smoke test, while the 100,000-row, 200-batch fixture owns the sustained throughput budget; both require complete acceptance with no errors or retries.

Immutable workflow artifacts checkpoint the exact deployment inventory before creation, the synthetic scope before ingestion, the prior read plane before promotion, provider ingestion, browser delivery, collector delivery, durable server delivery, and the ready-to-finalize state before cleanup. Collector measurements and scoped identities are persisted before their assertions can fail. The same workflow has an always-run recovery job that resolves uncertain creation by exact deployment-ID set difference, restores and validates the prior deployment, resumes paused Copies, cleans the owned synthetic scope from Tinybird and MySQL, and discards only the owned candidate. If Vercel has advanced the protected branch alias, only the database-cleanup route accepts the signed historical SHA; it separately attests that the serving deployment is the reviewed feature branch with the exact staging database and Tinybird configuration, and it keeps the cleanup bounded to the signed primary run ID plus at most seven validated related scopes. A force-cancel or GitHub control-plane outage can prevent that recovery job from starting; use the last immutable checkpoint for the manual recovery procedure.
