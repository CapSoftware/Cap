...
Immutable workflow artifacts checkpoint the exact deployment inventory before creation, the synthetic scope before ingestion, the prior read plane before promotion, provider ingestion, browser delivery, collector delivery, durable server delivery, and the ready-to-finalize state before cleanup. Collector measurements and scoped identities are persisted before their assertions can fail. The same workflow has an always-run recovery job that resolves uncertain creation by exact deployment-ID set difference, restores and validates the prior deployment, resumes paused Copies, cleans the owned synthetic scope from Tinybird and MySQL, and discards only the owned candidate. A force-cancel or GitHub control-plane outage can prevent that recovery job from starting; use the last immutable checkpoint for the manual recovery procedure.
