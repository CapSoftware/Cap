...
async function validateTables(client) {
  ...
  for (const table of TABLE_DEFINITIONS) {
    try {
      ...
      if (!remoteSchema) {
        issues.push(`Data source ${table.name} does not expose schema metadata`);
        continue;
      }
      ...
      const missingColumns = expectedColumns.filter((column, index) => remoteColumns[index] !== column);
      ...
      if (table.engine) {
        const actualEngine = typeof datasource.engine === "string" 
          ? datasource.engine 
  ...
  for (const pipeDef of PIPE_DEFINITIONS) {
    try {
      ...
      if ((pipe.type || "").toLowerCase() !== "materialized") {
        issues.push(`Pipe ${pipeDef.name} is not materialized (type=${pipe.type ?? "unknown"}).`);
        continue;
      }

      const materializedNode = pipe.nodes?.find((node) =>
      ...
      const errorMessage = error instanceof Error 
        ? error.message 
        : String(error);
      const errorDetails = error instanceof Error && error.cause 
        ? ` (cause: ${error.cause})` 
        : "";
      issues.push(
...
async function main() {
  try {
    ...
    console.error("❌ Tinybird analytics validation crashed:", error instanceof Error ? error.message : error);
    process.exit(1);
  }
}
