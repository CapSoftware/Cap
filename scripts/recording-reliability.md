...
Research dates: 2026-08-30–31. Source baseline: official `cap-v0.5.9`, `c6b83804d2e9fa8757b75268e573b8ff113141bb`. Initial audited candidate: `526dc80bf5429f7729b66d0291802b65cfeb48c7`, PR #2172; subsequent branch hardening is described below. This document distinguishes design requirements from implemented behavior and measured results. It is not a release approval.
...
The initial changes protect atomic recording metadata replacement with Windows open-reader support, Tauri segment completion, restart inventories and exact multipart retry reads. They remove one redundant clean-Studio scan and add the common local recording oracle.

Further hardening separates local completion from background transfer in GPUI and Tauri. Persistent upload ledgers retain the video identity, capture intent, verification request and retry state across application restarts. An operating-system lock shared by both applications prevents concurrent ownership of the same project; cancellation and cleanup must preserve ownership until their child work has finished. Authentication requests stay bound to the account and server that own the recording. These changes do not replace capture adapters, codecs or the recording container.

The versioned completion contract distinguishes queued or processing work from a verified upload. Local retention cleanup requires a full remote decode, required audio coverage, the expected final manifest or MP4 upload identity, and a matching current storage object. The media server pins reads to the identity returned by the actual successful upload, checks it before and after decoding, and retains a bounded verification deadline independently of earlier mux work. Google Drive uses its file ID and monotonic version; S3 uses its opaque ETag, without assuming that ETags are content hashes. Failed webhook delivery can be reconciled through the same validated completion handler. This requires the matching web and media-server deployment before the new desktop cleanup contract can succeed.

CLI upload APIs currently acknowledge transfer or processing rather than producing that verified receipt. Upload-dependent automation therefore retains local recordings when verification is unavailable, including after an upload failure or an unverified `UploadCompleted` event. Other automation actions can continue. This is a safety guard, not evidence that CLI cloud completion has the same verified queue as the desktop applications.

Production package builds and focused tests remain separate from native acceptance. Real 0.5.9 baseline runs have exposed environment or existing behavior limits: fixed PulseAudio buffer negotiation on Linux, absent Windows CLI window enumeration without process DPI awareness, variable physical webcam cadence, and a microphone fixture that was too quiet. Preserve those observations and compare equivalent workloads; do not silently reduce acceptance thresholds or label infrastructure readiness as a Cap pass. Background retry, complete GUI coverage, editing/export, crash recovery and strict performance parity still require final-build evidence.
