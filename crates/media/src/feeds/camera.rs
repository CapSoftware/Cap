...
fn setup_camera(id: DeviceOrModelID) -> Result<SetupCameraState, SetupCameraError> {
    ...
    thread::spawn(move || {
        ...
        match capture_handle.native().inner() {
            cap_camera_windows::CaptureHandle::MediaFoundation(handle) => {
                for event in handle.event_rx().iter() {
                    match event.variant() {
                        Some(cap_camera_mediafoundation::CaptureEngineEventVariant::Error) => {}
                        _ => {}
                    }
                }
            }
            cap_camera_windows::CaptureHandle::DirectShow(handle) => loop {
                if let Ok(Some(e)) = handle.try_get_event(1000) {
                    dbg!(e);
                }
            },
        }
