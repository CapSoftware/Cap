...
async fn test_playback(
    ...
    let display_path = match meta {
        StudioRecordingMeta::SingleSegment { segment } => recording_meta.path(
            &segment
                .display
                .as_ref()
                .map(|d| d.path.clone())
        ...
        StudioRecordingMeta::MultipleSegments { inner } => recording_meta.path(
            &inner.segments[segment_index]
                .display
                .as_ref()
                .map(|d| d.path.clone())
                .unwrap_or_default(),
...
async fn run_tests_on_recording(
    ...
    let is_fragmented = match studio_meta.as_ref() {
        StudioRecordingMeta::SingleSegment { segment } => meta
            .path(
                &segment
                    .display
                    .as_ref()
    ...
    for segment_idx in 0..segment_count {
        if run_decoder {
            let display_path = match studio_meta.as_ref() {
                StudioRecordingMeta::SingleSegment { segment } => meta.path(
                    &segment
                        .display
                        .as_ref()
                        .map(|d| d.path.clone())
                ...
                StudioRecordingMeta::MultipleSegments { inner } => meta.path(
                    &inner.segments[segment_idx]
                        .display
                        .as_ref()
                        .map(|d| d.path.clone())
                        .unwrap_or_default(),
