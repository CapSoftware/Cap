...
struct MixerSource {
    ...
    buffer_last: Option<(SourceTimestamp, Duration)>,
}

pub struct AudioMixerBuilder {
    sources: Vec<MixerSource>,
    output: Sender<(ffmpeg::frame::Audio, SourceTimestamp)>,
}

impl AudioMixerBuilder {
    pub fn new(output: Sender<(ffmpeg::frame::Audio, SourceTimestamp)>) -> Self {
        Self {
            sources: Vec::new(),
            output,
        }
...
impl AudioMixer {
    ...
    pub const BUFFER_TIMEOUT: Duration = Duration::from_millis(5);

    fn buffer_sources(&mut self, start: SourceTimestamps) {
        for source in &mut self.sources {
            while let Ok((frame, timestamp)) = source.rx.try_recv() {
                ...
                if let Some((buffer_last_timestamp, buffer_last_duration)) = source.buffer_last {
                    ...
                    if timestamp_elapsed > buffer_last_elapsed {
                                ...
                                >= Duration::from_millis(1)
                        {
                            ...
                            let silence_samples_needed =
                                (gap.as_secs_f64()) * source.info.rate() as f64;
                            ...
                            source.buffer_last = Some((
                                ...
                                Duration::from_secs_f64(
                                    silence_samples_count as f64 / source.info.rate() as f64,
                                ),
                            ));
                    ...
                    let gap = timestamp.duration_since(start);
                    if !gap.is_zero() {
                        let silence_samples_needed = gap.as_secs_f64() * source.info.rate() as f64;
                        let silence_samples_needed = silence_samples_needed.ceil() as usize;
                        ...
                        // let timestamp = SourceTimestamp::Instant(start.instant());
                        // source.buffer_last = Some((
                        //     timestamp,
                        //     Duration::from_secs_f64(
                        //         silence_samples_needed as f64 / source.info.rate() as f64,
                        //     ),
                        // ));
    ...
    fn tick(&mut self, start: SourceTimestamps, now: Instant) -> Result<(), ()> {
        // if let Some(last_tick) = self.last_tick {
        //     let time_since_last_tick = now.duration_since(last_tick);
        //     if time_since_last_tick > Self::BUFFER_TIMEOUT {
        //         let gap = Self::BUFFER_TIMEOUT;
        //         for source in &mut self.sources {
        //             let silence_samples_needed = (gap.as_secs_f64()) * source.info.rate() as f64;
        //             let silence_samples_count = silence_samples_needed.ceil() as usize;
        ...
        //                 source.info.channel_layout(),
        //             );
        //             dbg!(source.info.sample_format);

        //             frame.set_rate(source.info.rate() as u32);

        //             source.buffer_last = Some((SourceTimestamp::Instant(last_tick), gap));
        //             source
        //                 .buffer
        //                 .push_back((frame, SourceTimestamp::Instant(last_tick)));
        //         }
        //     }
        // } else {
        //     let time_since_last_tick = now.duration_since(start.instant());
        //     if time_since_last_tick > Self::BUFFER_TIMEOUT {
        //         let gap = Self::BUFFER_TIMEOUT;
        //         for source in &mut self.sources {
        //             let silence_samples_needed = (gap.as_secs_f64()) * source.info.rate() as f64;
        ...
        //             source.buffer_last = Some((SourceTimestamp::Instant(start.instant()), gap));
        //             source
        //                 .buffer
        //                 .push_back((frame, SourceTimestamp::Instant(start.instant())));
        //         }
        //     }
        // }
        ...
        while self.abuffersink.sink().frame(&mut filtered).is_ok() {
            let timestamp = start.instant()
                + Duration::from_secs_f64(self.samples_out as f64 / filtered.rate() as f64);

            self.samples_out += filtered.samples();

            if self
                .output
                .send((filtered, SourceTimestamp::Instant(timestamp)))
                .is_err()
            {
                return Err(());
            }
    ...
    pub fn run(&mut self) {
        let start = SourceTimestamps::now();
    ...
    pub fn builder(output: Sender<(ffmpeg::frame::Audio, SourceTimestamp)>) -> AudioMixerBuilder {
...
mod test {
    ...
    fn mix_sources() {
        ...
        tx1.send((
            ...
            SourceTimestamp::Instant(start.instant()),
        ))
        .unwrap();
        tx2.send((
    ...
    mod source_buffer {
        ...
        fn frame_gap() {
            ...
            tx.send((
                SOURCE_INFO.wrap_frame(&vec![0; SAMPLES_SECOND / 2]),
                SourceTimestamp::Instant(start.instant() + ONE_SECOND),
            ))
            .unwrap();
        ...
        fn start_gap() {
            ...
            tx.send((
                SOURCE_INFO.wrap_frame(&vec![0; SAMPLES_SECOND / 2]),
                SourceTimestamp::Instant(start.instant() + ONE_SECOND / 2),
            ))
            .unwrap();
