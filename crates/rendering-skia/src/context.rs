...
impl SkiaRenderContext {
    ...
    pub fn create_surface(&mut self, width: u32, height: u32) -> Result<Surface, SkiaRenderingError> {
        if width == 0 || height == 0 {
            return Err(SkiaRenderingError::InvalidDimensions(
                format!("Invalid surface dimensions: {}x{}", width, height)
            ));
        }
        ...
        #[cfg(target_os = "macos")]
        {
            if let Some(ref mut gpu_context) = self.gpu_context {
                ...
                return gpu::surfaces::render_target(
                    ...
                    false,
                    false,
                ).ok_or_else(|| SkiaRenderingError::SurfaceCreationFailed(
                    format!("Failed to create {}x{} GPU surface", width, height)
                ));
            }
        }
        ...
        surfaces::raster_n32_premul((width as i32, height as i32))
            .ok_or_else(|| SkiaRenderingError::SurfaceCreationFailed(
                format!("Failed to create {}x{} surface", width, height)
            ))
    }
    ...
    pub fn test_render(&mut self, width: u32, height: u32) -> Result<Vec<u8>, SkiaRenderingError> {
        ...
        let shader = Shader::linear_gradient(
            (Point::new(0.0, 0.0), Point::new(width as f32, height as f32)),
            colors.as_slice(),
            None,
            TileMode::Clamp,
            None,
