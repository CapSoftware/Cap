...
pub struct SegmentMedia {
    ...
    pub keyboard: Arc<cap_project::keyboard::KeyboardEvents>,
    ...
    match &meta {
        cap_project::StudioRecordingMeta::SingleSegment { segment: s } => {
            ...
            Ok(vec![SegmentMedia {
                ...
                cursor,
                keyboard: Arc::new({
                    let kb_path = s.display.path.parent().map(|d| d.join("keyboard.json"));
                    kb_path
                        .and_then(|p| {
                            let full = recording_meta.path(&p);
                            full.exists()
                                .then(|| {
                                    cap_project::keyboard::KeyboardEvents::load_from_file(&full)
                                        .ok()
                                })
                                .flatten()
                        })
