...
impl UYVYToRGBA {
    pub async fn new(device: &wgpu::Device, queue: &wgpu::Queue) -> Result<Self, ConversionError> {
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("UYVY to RGBA Converter"),
        ...
        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("UYVY Converter Bind Group Layout"),
            entries: &[
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("UYVY Converter Pipeline Layout"),
            bind_group_layouts: &[&bind_group_layout],
            push_constant_ranges: &[],
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("UYVY Converter Pipeline"),
    ...
    pub fn convert_to_texture(
        ...
        let expected_size = (width * height * 2) as usize; // UYVY is 2 bytes per pixel
        ...
        // Create input buffer for UYVY data
        let input_buffer = self
            .device
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("UYVY Input Buffer"),
                contents: input_data,
        ...
        let uniform_buffer = self
            .device
            .create_buffer_init(&wgpu::util::BufferInitDescriptor {
                label: Some("UYVY Dimensions Buffer"),
        ...
        let mut encoder = self
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("UYVY Conversion Encoder"),
            });

        {
            let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("UYVY Conversion Pass"),
    ...
    pub fn convert(
        ...
        let mut encoder = self
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("UYVY Readback Encoder"),
            });
