...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Extract YUYV components based on alignment
    if (byte_offset == 0u) {
        // YUYV starts at word boundary: [YUYV]
        let word = input_buffer[word_index];
        let y0 = (word >> 0u) & 0xFFu;
        let u_val = (word >> 8u) & 0xFFu;
        let y1 = (word >> 16u) & 0xFFu;
        let v_val = (word >> 24u) & 0xFFu;

        y = select(y0, y1, is_odd_pixel);
        u = u_val;
        v = v_val;
    } else if (byte_offset == 1u) {
        // YUYV spans boundary: [?YUY][V???]
        let word0 = input_buffer[word_index];
        let word1 = input_buffer[word_index + 1u];
        let y0 = (word0 >> 8u) & 0xFFu;
        let u_val = (word0 >> 16u) & 0xFFu;
        let y1 = (word0 >> 24u) & 0xFFu;
        ...
        // YUYV spans boundary: [???Y][UYV?]
        let word0 = input_buffer[word_index];
        let word1 = input_buffer[word_index + 1u];
        let y0 = (word0 >> 24u) & 0xFFu;
        let u_val = (word1 >> 0u) & 0xFFu;
        let y1 = (word1 >> 8u) & 0xFFu;
        let v_val = (word1 >> 16u) & 0xFFu;
