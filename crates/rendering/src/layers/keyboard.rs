...
const BOUNCE_OFFSET_PIXELS: f64 = 6.0;
const PADDING_RATIO: f32 = 0.45;
const CORNER_RADIUS_RATIO: f32 = 0.5;
const Y_FACTOR_TOP: f32 = 0.08;
const Y_FACTOR_ABOVE_CAPTIONS: f32 = 0.75;
const Y_FACTOR_BOTTOM: f32 = 0.85;
...
impl FromStr for KeyboardPosition {
    type Err = ();

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            "top-left" => Ok(Self::TopLeft),
            "top-center" | "top" => Ok(Self::TopCenter),
            "top-right" => Ok(Self::TopRight),
            "bottom-left" => Ok(Self::BottomLeft),
            "bottom-right" => Ok(Self::BottomRight),
            "bottom-center" | "bottom" => Ok(Self::BottomCenter),
            "above-captions" => Ok(Self::AboveCaptions),
            _ => Err(()),
        }
    }
}
...
impl KeyboardPosition {
    fn y_factor(&self) -> f32 {
        match self {
            Self::TopLeft | Self::TopCenter | Self::TopRight => Y_FACTOR_TOP,
            Self::AboveCaptions => Y_FACTOR_ABOVE_CAPTIONS,
            Self::BottomLeft | Self::BottomCenter | Self::BottomRight => Y_FACTOR_BOTTOM,
        }
    }
}
...
impl KeyboardLayer {
    ...
    pub fn prepare(
        ...
        let position = active
            ...
            .position_override
            .as_deref()
            .unwrap_or(&settings.position)
            .parse::<KeyboardPosition>()
            .unwrap_or_default();
        ...
        let padding = font_size * PADDING_RATIO;
        let corner_radius = font_size * CORNER_RADIUS_RATIO;
        ...
        match self.text_renderer.prepare(
            ...
            Ok(_) => {}
            Err(e) => tracing::warn!("Error preparing keyboard text: {e:?}"),
        }
    ...
    pub fn render<'a>(&'a self, pass: &mut wgpu::RenderPass<'a>) {
        ...
        {
            Ok(_) => {}
            Err(e) => tracing::warn!("Error rendering keyboard text: {e:?}"),
        }
...
struct ActiveKeyboardSegment<'a> {
    segment: &'a cap_project::keyboard::KeyboardTrackSegment,
}

fn find_active_keyboard_segment<'a>(
    time: f64,
    segments: &'a [cap_project::keyboard::KeyboardTrackSegment],
...
fn build_visible_text(
    segment: &cap_project::keyboard::KeyboardTrackSegment,
    current_time: f64,
) -> String {
...
fn calculate_keyboard_bounce(current_time: f64, start: f64, end: f64, fade_duration: f64) -> f64 {
    ...
    if fade_in_progress < 1.0 {
        ...
        -(ease * ease) * BOUNCE_OFFSET_PIXELS
