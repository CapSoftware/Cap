...
export type AppInstallationSettingsRow =
	typeof Db.appInstallationSettings.$inferSelect;

export class AppInstallationSettingsRepo extends Effect.Service<AppInstallationSettingsRepo>()(
			...
			const deleteByInstallationId = (installationId: string) =>
				database.execute((db) =>
					db
						...
						.where(
							Dz.eq(Db.appInstallationSettings.installationId, installationId),
						),
				);
