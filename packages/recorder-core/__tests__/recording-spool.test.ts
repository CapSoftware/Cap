...
describe("RecordingSpool", () => {
	...
	it("skips recently updated spools so live cross-tab recordings are not offered as recovered", async () => {
		const backend = new MemoryRecordingSpoolBackend();
		await backend.initialize();
		const now = Date.now();
		const liveSession = {
		...
		await backend.createSession(liveSession);
		await backend.appendChunk(
			liveSession,
			0,
			new Blob(["live!"], { type: "video/webm" }),
		);
		const idleSession = {
			sessionId: "idle",
			mimeType: "video/webm",
		...
		await backend.createSession(idleSession);
		await backend.appendChunk(
			idleSession,
			0,
			new Blob(["idle!"], { type: "video/webm" }),
		);
		...
		expect(recovered.map((item) => item.sessionId)).toEqual(["idle"]);
		// The skipped live session keeps its data untouched.
		expect(backend.getSessionCount()).toBe(2);
		expect(backend.getChunkCount("live")).toBe(1);
	});

	it("touch refreshes updatedAt so paused recordings stay invisible to recovery sweeps", async () => {
		const backend = new MemoryRecordingSpoolBackend();
		const spool = await RecordingSpool.create(
			{ mimeType: "video/webm" },
			backend,
		...
		// Simulate a long pause: backdate the persisted session past the idle
		// threshold, then heartbeat. The sweep must skip the touched session.
		...
		expect(backend.getChunkCount(spool.sessionId)).toBe(1);
	});

	it("touch failures do not poison the spool write queue", async () => {
		const backend = new MemoryRecordingSpoolBackend();
		const spool = await RecordingSpool.create(
			{ mimeType: "video/webm" },
			backend,
		);
		await spool.appendChunk(new Blob(["one"], { type: "video/webm" }));
		await spool.flush();

		const originalCreateSession = backend.createSession.bind(backend);
		backend.createSession = async () => {
			throw new Error("touch write failed");
		};
		await expect(spool.touch()).resolves.toBeUndefined();
		backend.createSession = originalCreateSession;

		await spool.appendChunk(new Blob(["two"], { type: "video/webm" }));
		await spool.flush();
		expect(backend.getChunkCount(spool.sessionId)).toBe(2);
	});
