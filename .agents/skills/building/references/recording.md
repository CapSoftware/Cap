...
The runner dependency is pinned in this skill's package lock. Supply `DAYTONA_API_KEY` to the host process through existing secure configuration, not an argument or recipe. Recipe files and walkthrough code must be committed in the feature worktree. The sandbox receives `git archive` of the exact verified commit, not uncommitted local files. Review all setup/start/walkthrough commands before running them.
...
After creating a draft PR, record its URL using `pr`, then call `share`. Uploads are named `#<PR> — <feature> — <commit>` and moved into the root `PR's` folder in the user's default Cap organization. Folder identity is saved for retries. Never create duplicate folders or blindly retry an upload with an uncertain result. Reconcile by the saved title and metadata, then persist the discovered upload ID/link under the session lock.
